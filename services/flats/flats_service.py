...
class FlatsService:
    def __init__(
            self, search_executor: SearchExecutor, flats_repo: SeenFlatRepository,
            subscribe_repo: SubscriptionRepository) -> None:
    ...
    def remove_user_data(self, user_id: int) -> None:
        self._flats_repo.remove_by_user_id(telegram_user_id=user_id)
        self._subscribe_repo.remove_by_user_id(telegram_user_id=user_id)

    def save_flats(self, flats: list[SeenFlat]) -> None:
    ...
    def get_user_flats(self, user_id: int) -> list[SeenFlat]:
        return self._flats_repo.get_by_user_id(telegram_user_id=user_id)

    async def search_new_flats(self, user_id: str) -> list[Flat]:
    ...
    async def _select_new_flats(self, filters: SearchFilters, user_id: int) -> list[Flat]:
        ...
        seen_flat = self.get_user_flats(user_id=user_id)
