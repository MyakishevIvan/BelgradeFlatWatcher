...
from services.telegram.telegram_search_utils import build_filters, convert_to_model, \
...
class TelegramController:
    ...
    async def subscribe(self, update, context):
        ...
        self._flats_service.save_subscription(subscription)
    ...
    async def search(self, update, context) -> int:
        ...
        await update.callback_query.message.reply_text(
            'You can use /subscribe to receive updates once in a day',
            parse_mode="HTML")
    ...
    async def _send_new_flats(self, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            ...
            return
        logger.info('Flats found by subscription for userId=%s, flatsCount=%s', user_id,
                    len(new_flats))
        messages = try_create_flat_messages(flats=new_flats,
                                            error_message='There is no new flats today')
